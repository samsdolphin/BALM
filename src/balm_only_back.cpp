...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE*>& feature_map,
			   ...
			   Eigen::Matrix3d R_p, Eigen::Vector3d t_p, int f_type, int fnum, int capacity)
{
	...
	for (uint i = 0; i < pt_size; i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			loc_xyz[j] = pt_tran[j] / voxel_size[f_type];
			if (loc_xyz[j] < 0)
				loc_xyz[j] -= 1.0;
		}
		...
		{
			OCTO_TREE *ot = new OCTO_TREE(f_type, capacity);
			ot->plvec_orig[fnum]->push_back(pt_orig);
			ot->plvec_tran[fnum]->push_back(pt_tran);

			ot->voxel_center[0] = (0.5 + position.x) * voxel_size[f_type];
			ot->voxel_center[1] = (0.5 + position.y) * voxel_size[f_type];
			ot->voxel_center[2] = (0.5 + position.z) * voxel_size[f_type];
			ot->quater_length = voxel_size[f_type] / 4.0;
...
int main(int argc, char **argv)
{
	...
	int scan_count = 0, window_base = 0;
	...
	while(n.ok())
	{
		...
		ros::Time cur_t(full_buf.front()->header.stamp);
		...
		{
			t_poses.push_back(t_poses[scan_count-1] + q_poses[scan_count-1] * t_gather_pose);
			q_poses.push_back(q_poses[scan_count-1] * q_gather_pose);
		}

		parray.header.stamp = cur_t;
		geometry_msgs::Pose apose;
		apose.orientation.w = q_poses[scan_count].w();
		apose.orientation.x = q_poses[scan_count].x();
		apose.orientation.y = q_poses[scan_count].y();
		apose.orientation.z = q_poses[scan_count].z();
		apose.position.x = t_poses[scan_count].x();
		apose.position.y = t_poses[scan_count].y();
		apose.position.z = t_poses[scan_count].z();
		...
		int frame_head = scan_count-1 - window_base;
		cut_voxel(surf_map, pl_surf, q_poses[scan_count-1].matrix(), t_poses[scan_count-1], 0,
				  frame_head, window_size);
		cut_voxel(corn_map, pl_corn, q_poses[scan_count-1].matrix(), t_poses[scan_count-1], 1,
				  frame_head, window_size);
		...
		if (scan_count >= window_base + window_size)
		{
			...
			for (int i = 0; i < margi_size; i += pub_skip)
			{
				...
				pcl::transformPointCloud(*pl_full_buf[window_base + i], pc, trans);
				pc_send += pc;
			}
			...
			for (auto iter = surf_map.begin(); iter != surf_map.end(); ++iter)
			{
				...
				{
					iter->second->root_centers.clear();
					iter->second->marginalize(0, margi_size, q_poses, t_poses, window_base,
